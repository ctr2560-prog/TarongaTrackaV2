...
function buildEnglishObservationScore(text, animalId, classStage) {
  ...
  let vocabulary = 1;
  if (wc >= 3)                                                                 vocabulary = 2;
  if (wc >= 4 && (hasAnimalRef || deviceHits >= 1))                           vocabulary = 3;
  if (hasAnimalRef)                                                            vocabulary = Math.max(vocabulary, 4);
  if (hasAnimalRef && hasExplanation)                                          vocabulary = 5;

  // Understanding (detail slot) - did they demonstrate literacy understanding?
  let understanding = 1;
  if (wc >= 3)                                                                 understanding = 2;
  if (wc >= 4 && (hasAnimalRef || deviceHits >= 1))                           understanding = Math.max(understanding, 3);
  if (hasAnimalRef)                                                            understanding = Math.max(understanding, 4);
  if (hasAnimalRef && hasExplanation)                                          understanding = 5;
