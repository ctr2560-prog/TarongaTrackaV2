import { useState, useEffect, useCallback } from 'react';
import { createPortal } from 'react-dom';
import { useApp } from '../context/AppContext';

const ANALYTICS_STEPS = [
  {
    title: 'Analytics Overview',
    text: 'At a glance: total students, animals found, badges earned, and quiz scores. The chart below shows which animals your class visited most and how quiz results are distributed.',
    image: '/images/teacher-tutorial/analytics-overview.png',
  },
  {
    title: 'Writing & Insights',
    text: 'The radar chart breaks down writing quality across six domains. Teaching Takeaways highlight where your class excelled and where they need support — generated automatically from their work.',
    image: '/images/teacher-tutorial/analytics-writing.png',
  },
  {
    title: 'Student Performance',
    text: 'See every student ranked by overall performance band. Click any student to drill into their individual results, quiz answers, and observation writing.',
    image: '/images/teacher-tutorial/analytics-students.png',
  },
  {
    title: 'Viewing Observations',
    text: "Read each student's observation writing exactly as they submitted it. Use this to assess quality and spot students who may need extra support.",
    image: '/images/teacher-tutorial/analytics-observations.png',
  },
  {
    title: 'Marking & Review',
    text: 'Override any automatically-generated score here. Use the dropdowns to adjust marks for quiz, writing, and engagement, then save — changes update instantly.',
    image: '/images/teacher-tutorial/analytics-marking.png',
  },
  {
    title: 'Conservation Statements',
    text: "Each student writes a personal conservation commitment at the end of their visit. Read them here — they're often the most meaningful part of the whole experience.",
    image: '/images/teacher-tutorial/analytics-conservation.png',
  },
];

const STEPS = [
  {
    type: 'welcome',
    title: 'Welcome to your Teacher Dashboard',
    text: "Here's a quick tour of the key features so you can get the most out of Taronga Tracka with your class.",
    selector: null,
  },
  {
    type: 'spotlight',
    title: 'Create a New Class',
    text: 'Start here. Fill in your school, stage, and class name, then enter your Taronga access code to generate a unique join code for your students.',
    selector: '#t-create-class-section',
  },
  {
    type: 'spotlight',
    title: 'Taronga Access Code',
    text: "This is provided by Taronga when you book your visit. It links your class to the right program and unlocks the app for your students.",
    selector: '#t-access-code-input',
  },
  {
    type: 'spotlight',
    title: 'Your Classes',
    text: 'All your created classes appear here. Click any class to start a live session, monitor student progress in real time, and view detailed analytics after the visit.',
    selector: '#lms-classes-section',
  },
  {
    type: 'analytics',
    title: 'Class Analytics',
    text: "After your visit, detailed analytics are available for every class. Let's walk through what's available.",
    slides: ANALYTICS_STEPS,
  },
  {
    type: 'spotlight',
    title: 'Share Your Feedback',
    text: "We're always improving Taronga Tracka. Use this button to let us know what's working, what's not, and what you'd love to see next.",
    selector: '#t-feedback-btn',
    elevateTarget: true,
  },
];

const PADDING = 14;

function getRect(selector) {
  if (!selector) return null;
  const el = document.querySelector(selector);
  if (!el) return null;
  const r = el.getBoundingClientRect();
  return { x: r.left - PADDING, y: r.top - PADDING, w: r.width + PADDING * 2, h: r.height + PADDING * 2 };
}

function shouldShow(email, demo) {
  if (!email) return false;
  if (demo || email === 'demo@zoo') return true;
  return localStorage.getItem(`teacherTutorial_${email}`) !== 'true';
}

export default function TeacherTutorial() {
  const { teacherEmail, demoMode } = useApp();
  const [done, setDone] = useState(() => !shouldShow(teacherEmail, demoMode));
  const [step, setStep] = useState(0);
  const [slide, setSlide] = useState(0);
  const [rect, setRect] = useState(null);

  const current = STEPS[step];
  const isFirst = step === 0;
  const isLast = step === STEPS.length - 1;
  const isAnalytics = current?.type === 'analytics';
  const analyticsSlides = isAnalytics ? current.slides : [];
  const isLastSlide = isAnalytics && slide === analyticsSlides.length - 1;

  const updateRect = useCallback(() => {
    if (current?.selector) setRect(getRect(current.selector));
    else setRect(null);
  }, [current?.selector]);

  useEffect(() => {
    if (done) return;
    updateRect();
    window.addEventListener('resize', updateRect);
    return () => window.removeEventListener('resize', updateRect);
  }, [done, updateRect]);

  useEffect(() => {
    if (done || !current?.elevateTarget || !current?.selector) return;
    const el = document.querySelector(current.selector);
    if (!el) return;
    const prev = el.style.zIndex;
    el.style.zIndex = '3004';
    return () => { el.style.zIndex = prev; };
  }, [done, step, current?.elevateTarget, current?.selector]);

  const complete = () => {
    const isDemo = demoMode || teacherEmail === 'demo@zoo';
    if (!isDemo && STORAGE_KEY) localStorage.setItem(STORAGE_KEY, 'true');
    setDone(true);
  };

  const next = () => {
    if (isAnalytics && !isLastSlide) { setSlide(s => s + 1); return; }
    if (isLast) { complete(); return; }
    setSlide(0);
    setStep(s => s + 1);
  };

  const back = () => {
    if (isAnalytics && slide > 0) { setSlide(s => s - 1); return; }
    setSlide(0);
    setStep(s => s - 1);
  };

  if (done) return null;

  const totalDots = STEPS.length;
  const showSpotlight = current?.type === 'spotlight' && rect;

  return createPortal(
    <>
      {/* Overlay */}
      {current?.type === 'welcome' ? (
        <div style={{ position: 'fixed', inset: 0, zIndex: 3000, background: 'rgba(0,0,0,0.72)' }} />
      ) : (
        <svg style={{ position: 'fixed', inset: 0, width: '100%', height: '100%', zIndex: 3000, pointerEvents: 'none' }}>
          <defs>
            <mask id="tut-teacher-mask">
              <rect width="100%" height="100%" fill="white" />
              {showSpotlight && <rect x={rect.x} y={rect.y} width={rect.w} height={rect.h} rx="10" fill="black" />}
            </mask>
          </defs>
          <rect width="100%" height="100%" fill="rgba(0,0,0,0.72)" mask="url(#tut-teacher-mask)" />
          {showSpotlight && (
            <rect x={rect.x} y={rect.y} width={rect.w} height={rect.h} rx="10"
              fill="none" stroke="#2E7D32" strokeWidth="2.5"
              style={{ animation: 'ttPulse 1.8s ease-in-out infinite' }} />
          )}
        </svg>
      )}

      {/* Skip */}
      {!isFirst && (
        <button onClick={complete} style={{
          position: 'fixed', top: '1rem', right: '1rem', zIndex: 3003,
          background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.25)',
          color: 'rgba(255,255,255,0.85)', borderRadius: '999px', padding: '0.3rem 0.9rem',
          fontSize: '0.78rem', fontWeight: 600, cursor: 'pointer', backdropFilter: 'blur(6px)',
          transition: 'background 0.15s',
        }}
          onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.22)'}
          onMouseLeave={e => e.currentTarget.style.background = 'rgba(255,255,255,0.12)'}
        >Skip tour</button>
      )}

      {/* Welcome card */}
      {current?.type === 'welcome' && (
        <div style={{
          position: 'fixed', zIndex: 3002, left: '50%', top: '50%',
          transform: 'translate(-50%, -50%)',
          width: 'min(420px, calc(100vw - 2rem))',
          background: 'white', borderRadius: '20px', padding: '2rem 2rem 1.6rem',
          boxShadow: '0 20px 60px rgba(0,0,0,0.4)',
          animation: 'ttFadeIn 0.3s ease',
          textAlign: 'center',
        }}>
          <div style={{
            width: '56px', height: '56px', borderRadius: '16px', margin: '0 auto 1.25rem',
            background: 'linear-gradient(135deg, #1A5238, #2E7D32)',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            boxShadow: '0 4px 16px rgba(46,125,50,0.35)',
          }}>
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none">
              <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z" fill="white"/>
            </svg>
          </div>
          <div style={{ fontWeight: 800, fontSize: '1.4rem', color: '#1A5238', marginBottom: '0.6rem', lineHeight: 1.2 }}>
            {current.title}
          </div>
          <p style={{ margin: '0 0 1.5rem', fontSize: '0.9rem', color: '#555', lineHeight: 1.65 }}>
            {current.text}
          </p>
          <div style={{ display: 'flex', gap: '4px', justifyContent: 'center', marginBottom: '1.25rem' }}>
            {Array.from({ length: totalDots }).map((_, i) => (
              <div key={i} style={{ width: i === step ? '20px' : '7px', height: '7px', borderRadius: '999px', background: i === step ? '#2E7D32' : '#D1D5DB', transition: 'all 0.25s' }} />
            ))}
          </div>
          <button onClick={next} style={{
            width: '100%', padding: '0.9rem', borderRadius: '999px', border: 'none',
            background: 'linear-gradient(135deg, #1A5238, #2E7D32)', color: 'white',
            fontSize: '1rem', fontWeight: 700, cursor: 'pointer',
            boxShadow: '0 4px 16px rgba(46,125,50,0.4)',
          }}>Start Tour</button>
        </div>
      )}

      {/* Spotlight coach mark */}
      {current?.type === 'spotlight' && (
        <CoachMark
          step={step}
          totalDots={totalDots}
          title={current.title}
          text={current.text}
          rect={rect}
          isLast={isLast}
          onBack={back}
          onNext={next}
          onSkip={complete}
        />
      )}

      {/* Analytics slideshow */}
      {isAnalytics && (
        <AnalyticsModal
          step={step}
          totalDots={totalDots}
          slide={slide}
          slides={analyticsSlides}
          isLast={isLast}
          isLastSlide={isLastSlide}
          onBack={back}
          onNext={next}
          onSkip={complete}
        />
      )}

      <style>{`
        @keyframes ttFadeIn {
          from { opacity: 0; transform: translate(-50%, -46%); }
          to   { opacity: 1; transform: translate(-50%, -50%); }
        }
        @keyframes ttSlideUp {
          from { opacity: 0; transform: translateY(10px); }
          to   { opacity: 1; transform: translateY(0); }
        }
        @keyframes ttPulse {
          0%, 100% { stroke-opacity: 1;   stroke-width: 2.5; }
          50%       { stroke-opacity: 0.5; stroke-width: 4; }
        }
      `}</style>
    </>,
    document.body
  );
}

function CoachMark({ step, totalDots, title, text, rect, isLast, onBack, onNext }) {
  const vw = window.innerWidth;
  const vh = window.innerHeight;

  let cardStyle = {};
  const cardW = Math.min(300, vw - 32);

  if (rect) {
    const spaceBelow = vh - (rect.y + rect.h);
    const spaceAbove = rect.y;
    if (spaceBelow >= 180) {
      cardStyle = { top: `${rect.y + rect.h + 14}px`, left: `${Math.min(Math.max(rect.x, 16), vw - cardW - 16)}px` };
    } else if (spaceAbove >= 180) {
      cardStyle = { bottom: `${vh - rect.y + 14}px`, left: `${Math.min(Math.max(rect.x, 16), vw - cardW - 16)}px` };
    } else {
      cardStyle = { top: '50%', left: '50%', transform: 'translate(-50%, -50%)' };
    }
  } else {
    cardStyle = { top: '50%', left: '50%', transform: 'translate(-50%, -50%)' };
  }

  return (
    <div style={{
      position: 'fixed', zIndex: 3002,
      width: `${cardW}px`,
      background: 'white', borderRadius: '16px',
      padding: '1.1rem 1.2rem 1rem',
      boxShadow: '0 10px 40px rgba(0,0,0,0.3)',
      animation: 'ttSlideUp 0.22s ease',
      ...cardStyle,
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.4rem' }}>
        <div style={{ width: '28px', height: '28px', borderRadius: '8px', background: 'linear-gradient(135deg, #1A5238, #2E7D32)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <span style={{ color: 'white', fontSize: '0.75rem', fontWeight: 800 }}>{step}</span>
        </div>
        <div style={{ fontWeight: 800, fontSize: '0.9rem', color: '#1A5238', lineHeight: 1.2 }}>{title}</div>
      </div>
      <p style={{ margin: '0 0 0.9rem', fontSize: '0.8rem', color: '#444', lineHeight: 1.6 }}>{text}</p>
      <div style={{ display: 'flex', gap: '4px', justifyContent: 'center', marginBottom: '0.75rem' }}>
        {Array.from({ length: totalDots }).map((_, i) => (
          <div key={i} style={{ width: i === step ? '14px' : '6px', height: '6px', borderRadius: '999px', background: i === step ? '#2E7D32' : '#D1D5DB', transition: 'all 0.2s' }} />
        ))}
      </div>
      <div style={{ display: 'flex', gap: '0.5rem' }}>
        <button onClick={onBack} style={{
          flex: 1, padding: '0.5rem', borderRadius: '999px',
          border: '1.5px solid #E5E5E5', background: 'white', color: '#555',
          fontSize: '0.78rem', fontWeight: 600, cursor: 'pointer',
        }}>Back</button>
        <button onClick={onNext} style={{
          flex: 2, padding: '0.5rem', borderRadius: '999px', border: 'none',
          background: 'linear-gradient(135deg, #1A5238, #2E7D32)', color: 'white',
          fontSize: '0.78rem', fontWeight: 700, cursor: 'pointer',
        }}>{isLast ? 'Done' : 'Next'}</button>
      </div>
    </div>
  );
}

function AnalyticsModal({ step, totalDots, slide, slides, isLast, isLastSlide, onBack, onNext }) {
  const current = slides[slide];

  return (
    <div style={{
      position: 'fixed', zIndex: 3002, left: '50%', top: '50%',
      transform: 'translate(-50%, -50%)',
      width: 'min(560px, calc(100vw - 2rem))',
      maxHeight: 'calc(100vh - 4rem)',
      background: 'white', borderRadius: '20px',
      boxShadow: '0 20px 60px rgba(0,0,0,0.4)',
      display: 'flex', flexDirection: 'column',
      overflow: 'hidden',
      animation: 'ttFadeIn 0.25s ease',
    }}>
      {/* Header */}
      <div style={{ background: 'linear-gradient(135deg, #1A5238, #2E7D32)', padding: '1rem 1.25rem', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.25rem' }}>
          <div style={{ background: 'rgba(255,255,255,0.2)', borderRadius: '6px', padding: '2px 8px', fontSize: '0.72rem', fontWeight: 700, color: 'white', letterSpacing: '0.05em' }}>
            ANALYTICS
          </div>
          <div style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.65)', fontWeight: 500 }}>
            {slide + 1} of {slides.length}
          </div>
        </div>
        <div style={{ fontWeight: 800, fontSize: '1rem', color: 'white', lineHeight: 1.2 }}>{current.title}</div>
      </div>

      {/* Screenshot */}
      <div style={{ background: '#f4f6f8', flexShrink: 0, maxHeight: '42vh', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <img
          src={current.image}
          alt={current.title}
          style={{ width: '100%', height: 'auto', display: 'block', objectFit: 'contain', maxHeight: '42vh' }}
        />
      </div>

      {/* Text + nav */}
      <div style={{ padding: '1rem 1.25rem 1.1rem', flexShrink: 0 }}>
        <p style={{ margin: '0 0 1rem', fontSize: '0.82rem', color: '#444', lineHeight: 1.6 }}>{current.text}</p>

        {/* Slide dots */}
        <div style={{ display: 'flex', gap: '4px', justifyContent: 'center', marginBottom: '0.85rem' }}>
          {slides.map((_, i) => (
            <div key={i} style={{ width: i === slide ? '14px' : '6px', height: '6px', borderRadius: '999px', background: i === slide ? '#2E7D32' : '#D1D5DB', transition: 'all 0.2s' }} />
          ))}
        </div>

        {/* Step dots */}
        <div style={{ display: 'flex', gap: '3px', justifyContent: 'center', marginBottom: '0.85rem' }}>
          {Array.from({ length: totalDots }).map((_, i) => (
            <div key={i} style={{ width: i === step ? '14px' : '5px', height: '5px', borderRadius: '999px', background: i === step ? 'rgba(46,125,50,0.4)' : '#E5E5E5', transition: 'all 0.2s' }} />
          ))}
        </div>

        <div style={{ display: 'flex', gap: '0.5rem' }}>
          <button onClick={onBack} style={{
            flex: 1, padding: '0.55rem', borderRadius: '999px',
            border: '1.5px solid #E5E5E5', background: 'white', color: '#555',
            fontSize: '0.8rem', fontWeight: 600, cursor: 'pointer',
          }}>Back</button>
          <button onClick={onNext} style={{
            flex: 2, padding: '0.55rem', borderRadius: '999px', border: 'none',
            background: 'linear-gradient(135deg, #1A5238, #2E7D32)', color: 'white',
            fontSize: '0.8rem', fontWeight: 700, cursor: 'pointer',
          }}>{isLast && isLastSlide ? 'Done' : isLastSlide ? 'Next' : 'Next'}</button>
        </div>
      </div>
    </div>
  );
}
