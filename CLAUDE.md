...
### Taronga Education ecosystem — shared login (Phase 2 complete, locally — not yet deployed)
Tracka is the foundation of a wider "Taronga Education" ecosystem — one login (email + password, chosen over magic-link because NSW DoE email filtering breaks link-based sign-in) that grants access to Tracka, **Wildly by Taronga** (`/Users/cameronrodgers/wildly`, separate repo, separate GitHub Pages hosting at `wildlybytaronga.com.au`), and future products. `TeacherLoginScreen.jsx` is branded accordingly (both product logos, "Multiple applications, one log-in") and every successful sign-in/registration merges `products: arrayUnion('tracka')` into the `teachers/{email}` doc — the collection is deliberately still called `teachers` (not `educators`), to avoid migrating live teacher data for a cosmetic rename.

**Phase 2**: Wildly's `src/firebase.js` / `.firebaserc` now point at this same `tarongatracka` Firebase project (no more `wildly-762f5`) — same Auth user pool, same Firestore. One email+password account works in both apps (though not true cross-domain SSO — each app still needs its own sign-in, just with the same credentials, since Tracka and Wildly live on different top-level domains). Wildly's identity model was reworked from `users/{uid}` to the shared `teachers/{email}`:
- `useSessionUser()` reads `teachers/{email}` instead of `users/{uid}`.
- `AboutYouPage` (Wildly's post-signup profile step: name/country/role/school) writes to `teachers/{email}` with `merge:true`, using `schoolName` (not `school`) to match Tracka's existing field name, plus `products: arrayUnion('wildly')`.
- The About-You redirect gate checks `profile?.role` specifically (not just "does a profile doc exist") — otherwise an existing Tracka-only teacher visiting Wildly for the first time would skip About You forever, since their `teachers/{email}` doc already exists but has no Wildly-specific fields.
- `useUsers()` (Wildly's staff console user list) now sorts client-side instead of via Firestore `orderBy("name")`, since Tracka-only teacher docs without a `name` field would otherwise be silently excluded from the query.
- Wildly's own `firestore.rules`/`firebase.json` were deleted from its repo — rules are now centrally owned here, in this repo's `firestore.rules`, under a "Wildly by Taronga" section with an `isWildlyStaff()` function checking `teachers/{email}.role`. **Never deploy Firestore rules from Wildly's repo** — it no longer has a local rules file, and this file is the only canonical copy for the whole shared project.
- No data migration was needed: Wildly's content collections (`contentItems`, `dashboardConfig`, etc.) all fall back to hardcoded JS defaults when Firestore is empty, and there were no real signed-up Wildly users yet.
